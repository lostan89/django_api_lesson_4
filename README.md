...
Ваш токен должен передаваться в программу через переменную *api_key*.

Лучше передавать его не напрямую, а через переменную окружения.

Создайте в папке проекта файл .env и пропишите токен туда.

```python
api_key = env.str("NASA_API_KEY") - токен берется из файла .env
```
